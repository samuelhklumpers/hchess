...
markAvailableMoves g (PlayerTouch c _) = do
    ...
    whenJust maybeSelected $ \ (a , _) ->
        forM_ [(i, j) | i <- [0..7], j <- [0..7]] $ \ b -> do
            ...
            when (isRight res) $ cause "SendMarkAvailableMove" (c, b)
...
pawnDoubleMove (PieceMove p x y) = do
    ...
    when (isNothing target && ax == bx && rank (snd p) 1 == ay && ay + 2 * d == by) $ do
        ...
        enpassant ?= (moveNumber currentTurn + 1 , (ax , ay + d), y)
        cause "PromotionCheck" $ PromotionCheck c y p
        cause "CheckedMovePiece" $ PieceMove p x y
...
pawnEP (PieceMove p x y) = do
    ...
    forM_ ep $ \ (epTurn , epSquare , pSquare) -> when (moveNumber currentTurn == epTurn && y == epSquare && abs (ax - bx) == 1 && ay + d == by) $ do
        cause "Take" $ Take p pSquare
        cause "PromotionCheck" $ PromotionCheck c y p
        cause "CheckedMovePiece" $ PieceMove p x y
...
disconnect :: HasCallStack => Chess Colour
disconnect c = do
...
serverRule :: TMVar (M.Map PlayerId Connection) -> Chess SendRaw
